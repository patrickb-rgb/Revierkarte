...
function updateFloatingButtons() {
  if (floatingCreateBtn) {
    floatingCreateBtn.classList.toggle("active", state.createMode);
  }
  if (floatingMoveBtn) {
    floatingMoveBtn.classList.toggle("active", state.moveMode);
  }
}
...
function markerSvg(marker, selected = false) {
  const width = Math.round(Math.max(20, Number(marker.rect_w) || 50));
  const height = Math.round(Math.max(20, Number(marker.rect_h) || 25));
  const shaftWidth = 7;
  const shaftHeight = 20;
  const strokeWidth = 2;

  const totalWidth = width;
  const totalHeight = height + shaftHeight;
  ...
  const shaftX = Math.round((width - shaftWidth) / 2) + 0.5;
  const shaftY = height;
  const shaftInnerW = shaftWidth - strokeWidth;

  const outline = selected ? "#1f6feb" : "#000000";
  ...
  const fontSize = Math.max(6, Number(marker.font_size) || 18);
  ...
  return `
    <svg xmlns="http://www.w3.org/2000/svg"
      ...
      <rect
        ...
        y="${shaftY}"
        width="${shaftInnerW}"
        height="${shaftHeight - strokeWidth / 2}"
        fill="${fill}"
        stroke="#000000"
...
function buildDivIcon(marker, selected = false) {
  ...
  return L.divIcon({
    ...
    iconAnchor: [Math.round(totalWidth / 2), totalHeight]
  });
}
...
function sortMarkers(markers) {
  return [...markers].sort((a, b) => {
    ...
    const prefixB = mb[1] || "";
    if (prefixA !== prefixB) {
      return prefixA.localeCompare(prefixB, "de");
    }

    const numA = Number(ma[2] || 0);
    const numB = Number(mb[2] || 0);
    return numA - numB;
  });
}
...
function renderMarkerList() {
  ...
  if (filter) {
    markers = markers.filter((m) =>
      String(m.text || "").toLowerCase().includes(filter)
    );
  }
...
function importJsonBackup(file) {
  ...
  reader.onload = () => {
    try {
      ...
      state.markers = normalizeMarkers(parsed);
      state.nextId = Math.max(1, ...state.markers.map((m) => Number(m.id) || 0)) + 1;
      state.selectedId = null;
...
function createMarkerAtLatLng(latlng) {
  ...
  const marker = {
    ...
    font_size: safeNumber(newFontSize.value, 18),
    rect_w: safeNumber(newWidth.value, 50),
    rect_h: safeNumber(newHeight.value, 25),
    lv95_e: e,
    lv95_n: n,
...
function createSeriesMarkers() {
  ...
  const nw3857 = L.CRS.EPSG3857.project(northWest);
  const se3857 = L.CRS.EPSG3857.project(southEast);
...
if (floatingPdfBtn) floatingPdfBtn.addEventListener("click", openMenuModal);
