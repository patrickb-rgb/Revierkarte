...
function markerSvg(marker, selected = false) {
  ...
  const rectW = Math.max(8, Number(marker.rect_w) || 120);
...
function buildDivIcon(marker, selected = false) {
  ...
  if (shape === "circle" || shape === "triangle") {
    const size = Math.max(8, Number(marker.circle_size || marker.symbol_size) || 80) + (selected ? 12 : 6);
  ...
  const width = Math.round(Math.max(8, Number(marker.rect_w) || 120));
  const height = Math.round(Math.max(8, Number(marker.rect_h) || 60));
  const shaftHeight = 20;
...
function applyChanges() {
  ...
  marker.circle_size = safeNumber(editSymbolSize?.value, marker.circle_size || 24);
...
function drawMarkersOnExportCanvas(canvas, areaName) {
  ...
  for (const marker of state.markers) {
    ...
    const rectW = Math.max(8, Math.round(marker.rect_w || 50));
    const rectH = Math.max(8, Math.round(marker.rect_h || 25));
    const shaftW = 7;
    const shaftH = 20;
